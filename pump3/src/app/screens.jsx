...
export function TodayScreen({ plan, go }) {
  ...
  return (
    <main className="screen">
      ...
      <button className="section-card action-card card-button" onClick={() => go('nutrition')}>
      ...
      <section className="section-card">
        ...
        <div className="mini-exercises">{workout.exercises.slice(0, 3).map((item) => <span key={item.exerciseId}>{item.names.he}</span>)}</div>
      </section>
    </main>
  );
}
...
export function TrainingScreen({ training, sex }) {
...
export function ProgressScreen({ profile }) {
  return (
    <main className="screen">
      <div className="screen-heading"><p className="eyebrow">שקילות</p><h1>התקדמות</h1><p>המסך מוכן לחיבור להיסטוריית השקילות האמיתית.</p></div>
      <section className="weight-hero"><span>משקל נוכחי</span><strong>{profile.weightKg} ק״ג</strong><small>יעד: {profile.targetWeightKg} ק״ג</small></section>
      <section className="section-card muted-card"><h2>שקילה שבועית</h2><p>אין יצירת נתוני דמה בתוך שכבת ה־UI. נקודות היסטוריה יוצגו רק מנתונים שמורים.</p></section>
    </main>
  );
}
